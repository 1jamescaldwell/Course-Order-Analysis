...
'''
# log needs:
# 2025-07-02 10:54:30.736874 Script ran with Error: Please select at least two courses
# user selection was: [BIOL 3000]
# Add: get qlik User ID
# General results: 

# View results:
# Load all data in, then user selects a User ID and date/time to view
# 	Force selection of only one
# Restrict to last few or last several years of results

# remove query from return statement?
...
if __name__ == "__main__":
    ...
    try:
        ...
        if (course_count < 2) or (course_count > 10):
            error_message = 'Error: Please select between 2 and 10 courses'
        else:
            try:
    ...
    log_path = os.path.join(main_folder_path, f'logs\\{date}_log.txt')
    f = open(log_path, "a")
    ...
    #     # import data
    #     courses_data = pd.read_parquet(f'{data_path}/Courses/Course_Grade_Data.parquet')
    #     # qlik_user_input = pd.read_csv(f'{data_path}/intermediate/UserParameters_qlik_user_output.csv')
    #     # filepathtesting = r'E:\UBI_Projects\Course Order\data\Selected Courses\ywe4kw_2025-07-08_09-37-48.csv'
    #     # qlik_user_input = pd.read_csv(filepathtesting)
    ...
    #     # except:
    #     #     error_message = 'Qlik selection file failed to import or no filepath given'
    #     #     print(error_message)
    #     #     raise

    #     # Collect course list (eg ['BIOL 3000', 'BIOL 3020']
    #     course_list = qlik_user_input['SelectedCourseAndTerm'].fillna('').str.strip().tolist()
    #     course_list = [c for c in course_list if str(c).strip()] # Make list empty if filled with "Missing value"

    #     ## Collect min cutoff course number. 
    #         # Convert to integer. If the input is not a number, default to 50
    ...
    #     ## Collect include repeats
    #         # Convert to boolean. If the input is not a yes or no, default to False
    #     include_repeats =  qlik_user_input['RepeatYesNo'].iloc[0]
    ...
    #             plot_data_cols_to_keep = ['Student System ID', 'Avg Grade','Course|Order w/ Count']
    #             with pd.ExcelWriter(f'{data_path}\\Results\\outputtesting.xlsx', engine='openpyxl') as writer:
    #                 plot_data[plot_data_cols_to_keep].to_excel(writer, sheet_name='Plot Data', index=False)
    #                 agg_result.to_excel(writer, sheet_name='Summary Table', index=False)
    #             error_message = 'No errors!'
    #         except:
