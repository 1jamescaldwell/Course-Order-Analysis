...
See documentation on github: https://github.com/1jamescaldwell/Course-Order-Analysis
'''
...
# Main Analysis function
    # Takes in course list and parameters
    # Outputs data for plotting and a summary table 
def analyze_course_order(course_df, course_list,min_cutoff_course_number=0,include_repeats=False):
    ...
    course_df = course_df[course_df['Official Grade'] != 'W'].copy()  # Remove withdraws. Revisit withdraws at a later time
        # Withdraws must be removed before the ranking by term (next line) or cases such as "Biol 3000 1, Biol 3020 3" will apppear since the withdraw was in between
    ...
    replace_map = {
        ...
        'D-': 0.7,
        'F': 0.0}
    course_df['Official Grade'] = course_df['Official Grade'].map(replace_map).copy()
    ...
    if query.shape[0] < 1: # if query was valid but just resulted in no students having taken that combination, return empty dataframes
        ...
        valid_courses = course_counts[course_counts >= min_cases].index # Keep courses with instances greater than the min number of cases
        ...
        agg_result = plot_data.drop(columns=['Course|Order w/ Count']).groupby('Course|Order').agg({'Avg Grade': ['count', 'mean']}) # Drop the Course|Order w/ count column then calculate count and mean gpa of courses in final list
...
if __name__ == "__main__":
    ...
    top_folder_path = os.path.dirname(os.path.dirname(script_path)) # Goes up two from script location
    data_path = os.path.join(top_folder_path, 'Course Order Data')
    ...
    try:
        ...
        try:
            ...
            # error_message = 
            raise RuntimeError(r"Error: Qlik selection file failed to import or invalid file path given. Call the function like: python CourseOrder.py '..filepath\userID_2025-07-08_09-37-48.csv'")  # re-raise so outer except catches and prints the right message
        ...
        if str(e).startswith("Error:"):
            error_message = str(e)  # Any of the errors above
        else:
